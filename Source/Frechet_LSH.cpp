...
void LSH::DiscreteFrechet_create_new_curve(bucketList* node){

    int current_size = node->p->p_data.size();
    
    for(int i = 0; i < current_size; i++){

        double qx = floor(( abs((i+1) - t_x) )/delta + 0.5) * delta + t_x;
        node->new_curve.push_back(qx);

        double piy = node->p->p_data[i];
        double qy = floor(abs(piy - t_y)/delta + 0.5) * delta + t_y;
    ...
    while(current_element_x < node->new_curve.size()){
...
int LSH::DiscreteFrechet_gp_cluster(data* node){ // CALCULATE G (BUCKET NUMBER) FOR CLUSTERS 
    ...
    ContinuousFrechet_create_new_curve(&node_temp);
...
vector<data> LSH::DiscreteFrechet_kNNsearch(vector<double> q, int d, int N){ //FIND N NEAREST NEIGHBORS
    ...
    DiscreteFrechet_create_new_curve(&node_temp);
   
    current_q_bucket = DiscreteFrechet_gp(&node_temp);//G FOR THE CURRENT QUERY POINT 
...
void LSH::ContinuousFrechet_hashPush(data* p){ //PUSH POINTS IN HASH TABLE
...
void LSH::ContinuousFrechet_create_new_curve(bucketList* node){

    int current_size = node->p->p_data.size();

    for(int i = 0; i < current_size; i++)
       node->new_curve.push_back(floor( (node->p->p_data[i] + t_x)/delta) * delta);

    
    int current_element = 1;

    while(current_element < node->new_curve.size() - 1){
...
int LSH::ContinuousFrechet_gp(bucketList* node){//CALCULATE G (BUCKET NUMBER) FOR EACH POINT
...
vector<data> LSH::ContinuousFrechet_kNNsearch(vector<double> q, int d, int N){ //FIND N NEAREST NEIGHBORS
    ...
    current_q_bucket = ContinuousFrechet_gp(&node_temp);//G FOR THE CURRENT QUERY POINT 
    current_ID = node_temp.ID;
...
// vector<data*> LSH::ContinuousFrechet_range_search(vector<double> q, int d, int minR, int maxR){ //RANGE SEARCH
//     vector<data*> R_arr;//ARRAY FOR THE POINTS WITH DISTANCE >MINR AND <= MAXR
//     data temp;
