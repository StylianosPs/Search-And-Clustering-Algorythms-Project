...
vector<double> Clustering::Mean_Curve(vector<double> x, vector<double> y, int d){

    int m1 = x.size();
    int m2 = y.size();

    vector<vector<double>> Dynamic_Array(m1, vector<double> (m2));
    double distance = 0.0;
    ...
    while(Pi > 0 && Qi > 0){
        ...
        mean_curve.push_back((x[0] + y[0])/2);
       
    }
    cluster_filtering(mean_curve, d);

    return mean_curve;
}

data* Clustering::Frechet_new_centroid(int i, int d){//CALCULATE NEAREST CENTROID
...
void Clustering::Frechet_Di_first_centroid(int d){//INITIALIZE THE Di(DISTANCE BETWEEN POINT AND CENTROID) FOR EACH POINT

    clusterList centroid = cluster_Table[0].front();    

    double max = Discrete_Frechet_Distance(cluster_Table->begin()->p->p_data, centroid.p->p_data); //MAX DISTANCE BETWEEN CENTROID AND POINT
...
void Clustering::Frechet_k_means(int d, int n, vector<data*> dataList){//K_MEANS++
    ...
    for(int t = 0; t < K - 1; t++){//ASSIGN ALL CENTROIDS FOR THE FIRST TIME
        ...
        Frechet_table_Push(dataList, t + 2);//ASSIGN ALL THE POINTS TO THE NEAREST CENTROID

        P_array.clear(); //DAME ALLAGI AN XRIASTEI
...
void Clustering::Frechet_Lloyds(int d, vector<data*> dataList){//LLOYDS
    ...
    while(iteration_flag < 7){//STOP THE CENTROIDS FROM UPDATING IF THE DISTANCES BETWEEN THE CURRENT AND NEW CENTROIDS ITS ZERO
        ...
        Frechet_table_Push(dataList, K);  //ASSING ALL THE POINTS TO THE NEAREST CENTROID    
    }

}

void Clustering::Frechet_table_Push(vector<data*> dataList, int t){//ASSIGN ALL THE POINTS FROM DATALIST TO THE NEAREST CENTROID
...
void Clustering::Frechet_table_Push_reverse(vector<data*> dataList, int t){//ASSIGN THE POINTS TO CENTROIDS FOR THE REVERSE LSH/HYPERCUBE
...
void Clustering::reverse_range_search_LSH_Frechet(LSH* objLSH, int L, int R, vector<data*> dataList){//REVERS RANGE SEARCH LSH
    ...
    while(iteration_flag < 7){//STOP THE CENTROIDS FROM UPDATING IF THE DISTANCES BETWEEN THE CURRENT AND NEW CENTROIDS ITS ZERO
        ...
        while(radii_flag < 1){//STOP INCREASING RANGE IF THE ALL CLUSTERS GET NO NEW POINT
            radii_flag = 0;
        ...
        Frechet_table_Push_reverse(dataList, K);//ASSIGN THE REMAIN POINTS TO THE NEAREST CLUSTER
...
void Clustering::cluster_filtering(vector<double>& mean_curve, int d){

    double e = 1.0;
    while(mean_curve.size() > d){
