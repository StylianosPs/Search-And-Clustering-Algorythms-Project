...
int main(int argc, char *argv[]){
    ...
    if(argc == 21){
        ...
        if(algorithm == "Frechet"){
            ...
            delta = atoi(argv[18]); //na alla3o to argv number an to piani apo grammi entolon
    }
    else{
    ...
    while(getline(infile, line)){//FOR EACH INPUT POINT
        ...
        if(algorithm == "Hypercube")
                obj_Hyper.hashPush(dataList[dataList.size() - 1]);//PUSH POINT IN HASH TABLE
    ...
    while(getline(infileq, lineq)){//FOR EACH QUERY POINT
        ...
        else if(algorithm == "Hypercube"){cout << "DAME" <<endl;
